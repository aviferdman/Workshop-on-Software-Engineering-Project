using System;
using System.Collections.Generic;
using System.Text;

namespace AcceptanceTests
{
    /// <summary>
    /// Manages all of the shared data in one place
    /// </summary>
    public static class SharedTestsConstants
    {
        public const string USER_OTHER_NAME = "user123";
        public const string USER_OTHER_PASSWORD = "mypassword1";

        public const string USER_SHOP_OWNER_NAME = "shopowner78";
        public const string USER_SHOP_OWNER_PASSWORD = "shopowner_pass78";

        public const string USER_SHOP_OWNER_2_NAME = "nymn1";
        public const string USER_SHOP_OWNER_2_PASSWORD = "go5minutes";

        public const string USER_BUYER_NAME = "buyer78";
        public const string USER_BUYER_PASSWORD = "buyer_pass78";

        public const string SHOP_NAME = "my shop 1";
        public const string SHOP_NAME_2 = "another shop";
    }
}
