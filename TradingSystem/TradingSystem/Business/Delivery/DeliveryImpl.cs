...
namespace TradingSystem.Business.Delivery
{
    public class DeliveryImpl : DeliveryAdapter
    {
        ...
        public async Task<DeliveryStatus> CancelDelivery(DeliveryStatus deliveryStatus)
        {
            ...
            return new DeliveryStatus(packageId, deliveryStatus.Username, deliveryStatus.StoreId, !packageId.ToString().Equals(ErrorPackageId));
        }
        ...
        {
            string packageId = await _deliverySystem.CreateDelivery(deliveryDetails.Username, deliveryDetails.RecieverPhone, deliveryDetails.Weight, deliveryDetails.Source.ToString(), deliveryDetails.Destination.ToString());
